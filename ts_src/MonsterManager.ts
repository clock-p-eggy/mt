...
const AI_ATTACK_COOLDOWN = math.tofixed(1.2)
...
function nearestRoleInRange(position: Vector3, maxRange: Fixed): Role | undefined {
  ...
  for (const role of GameAPI.get_all_valid_roles()) {
    ...
    if (currentDistSq <= bestDistSq) {
      if (isLineBlockedBySceneObstacle(position, characterPos)) {
        continue
      }
...
function getChaseRole(data: MonsterData): Role | undefined {
  ...
  const role = PlayerStats.GetRoleById(data.chaseRoleId)
...
function canMonsterSeeRole(data: MonsterData, role: Role, maxRange: Fixed): boolean {
  ...
  const characterPos = character.get_position()
  return distSq(monsterPos, characterPos) <= maxRange * maxRange && !isLineBlockedBySceneObstacle(monsterPos, characterPos)
}
...
function commandChase(data: MonsterData, role: Role): void {
  ...
  data.unit.start_move_to_pos_with_threshold(targetPos, AI_TICK_SECONDS, AI_SCRIPT_STOP_RANGE)
  if (!wasSameTarget) {
...
function commandReturn(data: MonsterData): void {
  ...
  data.unit.start_move_to_pos_with_threshold(data.bornPos, AI_TICK_SECONDS, AI_RETURN_THRESHOLD)
  if (!wasReturning) {
...
function tryCollisionAttack(data: MonsterData, otherUnit: Unit | undefined): void {
  ...
  if (data.lastCollisionAttackTime !== undefined && aiClock - data.lastCollisionAttackTime < AI_ATTACK_COOLDOWN) {
    return
  }

  data.lastCollisionAttackTime = aiClock
  PlayerStats.ApplyDamage(role, math.tofixed(data.config.attack))
  role.show_tips(`${data.name} hit -${tostring(data.config.attack)} HP`, math.tofixed(1.2))
  print(
    `[Stage6][MonsterManager] collision attack ${data.name}` +
      ` role=${tostring(role.get_roleid())}` +
      ` damage=${tostring(data.config.attack)}`
  )
}
...
function syncEngineHp(data: MonsterData): void {
  const unit = data.unit
  const delta = data.hp - unit.get_hp()
  if (delta !== 0) {
...
function refreshDisplay(data: MonsterData): void {
  ...
  syncEngineHp(data)
  unit.set_name(`${data.name} ${hpText(data)}`)
  unit.set_name_visible(true)
  unit.set_mass_bar_visible(true)
  unit.set_attr_by_type(
    Enums.ValueType.HpBarDisplayMode,
    "HpBarDisplayMode",
    Enums.HpBarDisplayMode.PERSISTENT
  )
}
...
function reviveMonster(data: MonsterData): void {
  data.dead = false
  data.hp = data.config.maxHp
  data.expAwarded = false
  data.lastHitRoleId = undefined
  data.chaseRoleId = undefined
  data.lastCollisionAttackTime = undefined
  data.unit.set_position(data.bornPos)
  data.unit.reborn(true)
  ...
  refreshDisplay(data)
  print(`[Stage2][MonsterManager] revive ${data.name} hp=${hpText(data)}`)
}
...
function resolveRewardRole(data: MonsterData, role: Role | undefined): Role | undefined {
  ...
  if (data.lastHitRoleId === undefined) {
    return undefined
  }

  return PlayerStats.GetRoleById(data.lastHitRoleId)
}
...
function awardKillExp(data: MonsterData, role: Role | undefined): void {
  ...
  print(
    `[Stage4][MonsterManager] kill exp ${data.name}` +
      ...
      ` role=${tostring(rewardRole.get_roleid())}` +
...
function killMonster(data: MonsterData, role: Role | undefined): void {
  ...
  print(
    `[Stage2][MonsterManager] dead ${data.name}` +
      ` kind=${data.config.kind}` +
      ` revive=${tostring(data.config.reviveSeconds)}`
  )

  LuaAPI.call_delay_time(data.config.reviveSeconds, () => {
    reviveMonster(data)
  })
}
...
function handleEngineDamage(data: MonsterData, eventData: unknown): void {
  ...
  if (engineDamageInterceptor !== undefined && engineDamageInterceptor(data, eventData)) {
    return
  }
...
function initMonster(spec: MonsterSpec): void {
  ...
  data.dieEventId = LuaAPI.unit_register_trigger_event(unit, [EVENT.SPEC_LIFEENTITY_DIE], (_: string, __: unknown, eventData: unknown) => {
    ...
    recoverFromUnauthorizedEngineDeath(data, "restore engine death before scripted damage")
...
export function ApplyDamage(name: string, damage: Fixed): DamageResult | undefined {
...
export function ApplyDamageFromRole(name: string, role: Role, damage: Fixed): void {
  ...
  const result = ApplyDamage(name, damage)
  if (result !== undefined) {
